...
//****************************************************************************
char  CallSign[7]="N3VEM"; //DO NOT FORGET TO CHANGE YOUR CALLSIGN
...
char comment[50] = "N3VEM's Radio Rocket - www.n3vem.com/rocket"; // Max 50 char
char StatusMessage[50] = "waiting for GPS fix www.n3vem.com/rocket"; 
//*****************************************************************************


unsigned int   BeaconWait=0;  //seconds sleep for next beacon (TX).
...
void setup() {
  ...
  RfON; //default was off, but i think i want to just turn it on and leave it on for my use case
  GpsOFF;
  RfPwrLow;
  RfPttOFF;
  AprsPinInput;
  ...
  delay(2000); //added this hear, to make sure the chip is fully awake before configuring.
  ...
  if(aliveStatus){
    ...
    if ((gps.location.age() < 1000 || gps.location.isUpdated()) && gps.location.isValid()) {
      ...
      if(gps.time.minute() == 30){               
        //sendStatus();       
        sendLocation(); //I just want this always.
      } else {
